...
Ora configuro upsmon, dopo aver fatto un backup:
...
Vedo: 
    ...
    NOTIFYMSG ONLINE      "UPS %s connesso alla rete elettrica"
    NOTIFYMSG ONBATT      "UPS %s alimentato a batteria"
...
Contenuto attuale:
    ...
    #esegui come root
    MONITOR myUPS@localhost 1 upsadmin dsc master
    #monitora [nome]@localhost [numeroUPS] [account] [password] [master/slave]
    NOTIFYCMD /sbin/upssched
    #la path per il programma upssched, che usa come impostazioni upssched.conf 
    SHUTDOWNCMD "/sbin/shutdown -h +0"
    #questo dice a nut quale comando usare per lo spegnimento
    NOTIFYMSG ONLINE      "UPS %s connesso alla rete elettrica"
    NOTIFYMSG ONBATT      "UPS %s alimentato a batteria"
    #invio di messaggi al log
    NOTIFYFLAG ONLINE EXEC
    NOTIFYFLAG ONBATT EXEC
    #queste ultime due righe dicono a nut di eseguire il NOTIFYCMD quando avviene qualcosa
...
Contenuto attuale:
    ...
    #dove sta lo script da eseguire a fine timer
    PIPEFN /etc/nut/upssched.pipe
    LOCKFN /etc/nut/upssched.lock
    #file per la gestione dei timer
    AT ONBATT * START-TIMER onbatt 10
    #inizia un timer da N secondi  chiamato "onbatt" 
    AT ONLINE * CANCEL-TIMER onbatt online
    #elimina "onbatt" se si torna online
    AT ONBATT * START-TIMER earlyshutdown 900
    #inizia un timer da N secondi chiamato "earlyshutdown"
    AT ONLINE * CANCEL-TIMER earlyshutdown
    #quando si torna online, elimina il timer 
...
Contenuto di /etc/nut/upssched-cmd, ovvero CMDSCRIPT:
    ...
    case $1 in
        ...
        earlyshutdown)
            ...
            /usr/sbin/upsmon -c fsd #QUESTO INDICA UN FORCE SHUTDOWN
            ;;
        online)
...
## File di configurazione di cui fare il backup
### /etc/nut/ups.conf
Contiene la configurazione generale del mio UPS.

### /etc/nut/upsmon.conf
Contiene la configurazione di upsmon[itor], cioè l'utente da usare, cosa monitorare, il comando di spegnimento, cosa fare quando si verificano certi eventi. 

In questo file molto importante che compaia la path di upssched, che viene attivato da upsmon:

    NOTIFYCMD /sbin/upssched

### /etc/nut/upsd.conf
Contiene l'indicazione di come avviene l'ascolto:
- LISTEN 0.0.0.0 3493 per il monitoraggio sulla rete
- LISTEN 127.0.0.1 3493 per il monitoraggio in locale

### /etc/nut/nut.conf
Un'unica riga interessante:

    MODE=netserver

### /etc/nut/upsd.users
Imposta gli utenti e il loro ruolo:
...
Contiene l'impostazione dei timers e la path dello script da eseguire a fine timer.

### /etc/nut/upssched-cmd
Contiene lo script che viene eseguito quando i timer impostati nel file precedente giungono al termine.
...
## Guide
- Ben fatta: https://www.youtube.com/watch?app=desktop&v=vyBP7wpN72c&t=6m30s&ab_channel=TechnoTim
